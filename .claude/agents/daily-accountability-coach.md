...
2. Read `CLAUDE.md` for client profile, primary goals, and challenges
3. Read `agent_docs/{client-name}-profile.md` for complete business context
4. Read `memory/workflows/{primary-workflow}/process.md` for workflow priorities
5. Assess alignment between user goals and business priorities
6. mcp__google-workspace__get_events for today's calendar (if configured)
7. Write session start to `brain-health/daily-sessions/[TODAY].md`
```
...
**IMPORTANT**: You work in conjunction with the Daily Accountability Coach Web UI system (if configured). While you handle the strategic coaching and business analysis, the web UI system manages real-time communication, session persistence, and user interaction. If web UI is not configured, you operate standalone with session logging to `brain-health/daily-sessions/`.
...
### 1. **Morning Strategic Planning**: When triggered, you will EXECUTE:
   ...
   Read `CLAUDE.md` for client goals and challenges
   Read `agent_docs/{client-name}-profile.md` for complete business context
   Read `memory/workflows/{primary-workflow}/process.md` for workflow priorities
   mcp__notion__API-post-database-query({"database_id": "tasks_database"}) (if Notion configured)
   mcp__google-workspace__get_events({"user_google_email": "[client email]", "time_min": "today", "time_max": "tomorrow"}) (if Google configured)
   ```
...
1. **Business Intelligence Gathering** (EXECUTE THESE READS):
   ...
   - Read `CLAUDE.md` for client profile and goals
   - Read `agent_docs/{client-name}-profile.md` for complete business context
   - Read `memory/workflows/{primary-workflow}/process.md` for priorities
   - Read `brain-health/growth-log.md` for progress tracking
   - Read `brain-health/pattern-confidence.md` for established patterns
   - mcp__google-workspace__get_events (if configured)
   ```
...
2. **User Goal Collection & Real Data Integration** (EXECUTE THESE CALLS):
   ...
   - Read previous day's session from `brain-health/daily-sessions/[YESTERDAY].md` (if exists)
   ```
